...
import jins.db.whatsappgroup.tools.Emoji;
import jins.db.whatsappgroup.tools.EmojiOne;
...
import jins.db.whatsappgroup.tools.ImageCache;
...
public class EmojiListController {
	...
	private Node createEmojiNode(Emoji emoji) {
		...
		stackPane.setOnMouseClicked(e-> {
			if (messageInput != null) {
				messageInput.insertText(messageInput.getCaretPosition(), emoji.getUnicode());
			}
	...
	private String getEmojiImagePath(String hexStr) {
		return getClass().getResource("/jins/db/whatsappgroup/png_40/" + hexStr + ".png").toExternalForm();
	}
